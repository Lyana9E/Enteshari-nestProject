...
export function SimpleProductCard({sliderItem}: Props) {
    return (
        <>
            <div
                ...
                <Badge badge={sliderItem.label} price={sliderItem.price} salePrice={sliderItem.sale_price}/>
                <div
                    ...
                    <div className="p-2.5 border-r-[1px] border-r-green-200 hover:bg-green-150">
                        <IconBox icon={'icon-heart text-[15px] text-brand1'}/>
                    </div>
                    <div className="p-2.5 border-r-[1px] border-r-green-200 hover:bg-green-150">
                ...
                <ImageView src={sliderItem.imgSrc} className={'m-auto w-full aspect-[3/2] mb-[28px]'} width={210}
                           height={168} alt={'product image'}/>
                <div className="flex flex-col gap-2">
                    <div className="text-gray-500 text-xsmall">{sliderItem.category}</div>
                    <Link href={'./'}><h3
                        className="text-heading-sm text-blue-300 max-h-[50px] overflow-hidden">{sliderItem.title}</h3>
                    </Link>

                    <Rating ratingNumber={sliderItem.rate}/>
                    <div className="font-lato text-xsmall text-gray-500">{sliderItem.weight} {sliderItem.unit}</div>
                </div>

                {
                    sliderItem.total && sliderItem.sold ?
                        <>
                            ...
                            <div className="mt-[15px] bg-gray-200 h-[4px] w-full rounded-[2px]">
                                <div className={`bg-green-200 h-[4px]  rounded-[2px]`}></div>
                            </div>
                            <div
                                className="mt-2.5 font-lato text-blue-300 text-xsmall">Sold: {sliderItem.sold}/{sliderItem.total}</div>
                            <div className="mt-[23px]">
                                <button
                                    ...
                                    <IconBox icon={'icon-shopping-cart'} size={22}/>
